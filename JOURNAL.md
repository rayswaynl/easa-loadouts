...
- EASA build-2 ✅ (ef58c3c): JS parser = Python-grammar port; import modal; 4 exports (preset row w/ auto-label, new-vehicle block, FULL file splice, vehicle addWeaponTurret snippet w/ rearm caveat). I VERIFIED the no-op byte-identical gate myself against the real 81,301-char EASA_Init.sqf → identical:true. 49 unit + 22 Playwright green (note: test_playwright.py is a SCRIPT, run `python tools/test_playwright.py`, not pytest).
- EASA SHIPPED 🚀: repo github.com/rayswaynl/easa-loadouts + Pages LIVE at https://rayswaynl.github.io/easa-loadouts/ (cold-load probed on prod: seed 21 vehicles, 0 console errors). Hero screenshot docs/screenshots/hero-2026-07-02.png.
- HUB STAGED: miksuu branch feat/site-easa-tile (stacked on site-visual → site-improve): EASA tile in tools.ts + 7 REGENERATED thumbnails (1280×800 from overhaul screenshots). `npm run build` clean. Faction-builder thumb pending its overhaul merge. DEPLOY = one user-gated bundle: improve → visual → tile.
- faction-builder visual: review found APPROVED stamps rendering on FRESH session + covering tab labels → sent back for fix (logic: stamp only after genuine completion; placement: label must stay readable).
